...
├── package.json
├── SECURITY-AUDIT.md         # 安全审查报告 + 修复记录
├── .gitignore                # 忽略私钥、证书与构建产物
├── dist/
│   └── Markdown工具箱.exe     # 打包产物（约 13 MB 单文件）
├── src/
│   ├── taskpane.html/css/js  # 侧边任务面板（三个标签页）
│   ├── commands.html/js      # 功能区命令宿主
│   ├── dialog-export.html/js # 导出结果对话框
│   ├── dialog-paste.html/js  # 手动粘贴对话框
│   ├── dialog-error.html/js  # 错误提示对话框
│   ├── support.html          # 帮助页（清单 SupportUrl 指向此页）
│   ├── selftest.html/js      # 浏览器内自检页
│   └── core/
│       ├── zip.js            # 零依赖 ZIP + inflate 解压器（含解压上限）
│       ├── markdown.js       # Markdown → Word HTML 转换器（白名单净化）
...
- **安全回归（150 项）**：37 项 XSS 向量（含 `<svg/onload>`、斜杠分隔事件属性、`math/xlink`、CSS expression、form 注入）、18 项 HTML 实体编码绕过、路径穿越、畸形 URL、CORS、CSP、ZIP 解压上限、私钥保护

浏览器内自检：启动服务器后访问 `https://localhost:3000/selftest.html`，37 项检查全部通过。

安全审查报告见 [`SECURITY-AUDIT.md`](SECURITY-AUDIT.md)（v1.0.2 已修复全部 9 项漏洞）。
...
**Q：安全性如何？文档内容会上传吗？**
A：不会。全项目除 Office 官方 CDN 的 `office.js` 外无任何网络请求，文档内容全程留在本机。v1.0.2 已修复安全审查发现的全部 9 项问题，包括 XSS 净化改为**标签/属性白名单**（原正则黑名单可被 `<svg/onload>` 等绕过）、路径穿越、畸形 URL 崩溃、CORS 收紧、严格 CSP（已移除 `'unsafe-inline'`）、ZIP 解压炸弹上限、私钥文件保护。详见 [`SECURITY-AUDIT.md`](SECURITY-AUDIT.md)。

**Q：粘贴 Markdown 时会不会有脚本注入风险？**
A：不会。Markdown 转 HTML 后经过白名单净化：仅保留 `b/i/u/p/table/a/img` 等安全标签与 `href/src/alt/title/style` 等安全属性，`script`/`svg`/`math`/`iframe`/`form` 连内容整体丢弃，`javascript:` 等危险协议（含 HTML 实体编码变体）一律剥离。该结论经真实浏览器注入执行验证，零执行。
