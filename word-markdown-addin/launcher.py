...
def ensure_certificate() -> tuple[bytes, bytes]:
    ...
    # 收紧私钥文件权限：仅当前用户可读写（Windows 上默认继承目录 ACL，
    # 而 %LOCALAPPDATA% 通常已是用户私有，此处再显式收紧一道）
    try:
...
def _restrict_key_acl(key_path: Path) -> None:
    """把私钥文件的 ACL 限制为「仅当前用户」，移除继承权限。

    失败不影响功能（%LOCALAPPDATA% 本身已是用户私有目录），因此静默忽略。
    """
    try:
        import subprocess
...
class AddinHandler(http.server.BaseHTTPRequestHandler):
    """静态文件处理器，严格限定在 src/ 与 assets/ 目录内。

    注意：绝不能把 BUNDLE（源码模式下即项目根目录）加入 roots ——
    那会让 .certs/key.pem、launcher.py 等敏感文件可被直接下载。
    """
    ...
    def _resolve(self, pathname: str) -> Path | None:
        ...
        for root in self.roots:
            try:
                ...
                candidate.relative_to(base)  # 防目录穿越
    ...
    def _send_common_headers(self, content_type: str, length: int | None = None) -> None:
        self.send_header("Content-Type", content_type)
        self.send_header("Cache-Control", "no-store")
        self.send_header("X-Content-Type-Options", "nosniff")
        # 同源部署无需 CORS；仅回显本机来源，避免任意网页跨源读取
    ...
    def do_GET(self):  # noqa: N802
        ...
        self._send_common_headers(
            MIME.get(file.suffix.lower(), "application/octet-stream"), len(data)
        )
        self.end_headers()
...
def start_server(port: int, use_https: bool) -> ReusableTCPServer:
    # 只暴露 src/ 与 assets/：不再加入 BUNDLE（源码模式下 BUNDLE 是项目根，
    # 会把 .certs/key.pem、launcher.py 一并暴露出去）。
    AddinHandler.roots = [SRC_DIR, ASSETS_DIR]
    scheme = "https" if use_https else "http"
    AddinHandler.allowed_origin = f"{scheme}://localhost:{port}"
    if use_https:
        import ssl
